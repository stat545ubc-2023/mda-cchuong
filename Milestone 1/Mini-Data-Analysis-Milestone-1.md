...
- 32 columns/variables (The top of the output lists the number of
  columns and there is one variable per column)

- All variables are numeric/doubles except diagnosis (Next to the
  variable name, the class of the variable is given in the form
  \<class\> )
...
- 7 columns/variables (The top of the output lists the number of columns
  and there is one variable per column)
...
- 21 columns/variables (The top of the output lists the number of
  columns and there is one variable per column)
...
- 20 columns/variables (The top of the output lists the number of
  columns and there is one variable per column)
...
observations and columns, so there is lots to explore, doesn’t look too
messy (not a lot of NA’s or long strings in the cells I need to parse),
and there is a categorical variable so I can do different kinds of
analyses for the different categorical variable levels.
<!----------------------------------------------------------------------------->
...
Is there a relationship between being diagnosed with a malignant tumor
(M) and mean nuclei area?

Or, in terms of the variables, does radius_mean differ between patients
with diagnosis=“M” and patients with diagnosis=“B”?
<!----------------------------------------------------------------------------->
...
so I could create many different plots. And it makes sense to plot the
distribution of area_mean to see the range of mean nuclei sizes.
...
area_mean and see how the values of the area_mean differ based on
diagnosis.
...
I chose this exercise because I may want to explore the relationships or
distributions of variables for people diagnosed with malignant tumors,
so it makes sense to only use the observations where patients were
diagnosed with malignant tumors (diagnosis=“M”)

## Exercise 4: Make a new tibble with a subset of your data

I am going to remove the columns with the “se” variables
...
I chose this exercise because my dataset has many columns, and since I
am not investigating the standard errors of the variables, I won’t be
using all the columns with “se” in them, so it makes sense to remove
them.
...
3.  What variables can be used to predict mean texture (texture_mean)?
